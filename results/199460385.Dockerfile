[app/sources/199460385.Dockerfile]
digraph {
  ...
  "sha256:71b32f17a6f19a52fdc43dd003177093b5f113511928b5391dcfe40d1d2b059f" [label="/bin/sh -c set -x \t&& curl -fSL \"$TOMCAT_TGZ_URL\" -o tomcat.tar.gz \t&& curl -fSL \"$TOMCAT_TGZ_URL.asc\" -o tomcat.tar.gz.asc \t&& gpg --verify tomcat.tar.gz.asc \t&& tar -xvf tomcat.tar.gz --strip-components=1 \t&& rm bin/*.bat \t&& rm tomcat.tar.gz* \t&& cd /opt \t&& wget \"$CMDBUILD_URL\" \t&& mv cmdbuild* cmdbuild.zip \t&& unzip cmdbuild.zip \t&& rm -rf cmdbuild.zip \t&& mv cmdbuild* cmdbuild \t&& cd /usr/local/tomcat/webapps/ROOT \t&& rm -rf * \t&& cd /opt/cmdbuild/ \t&& mkdir temp \t&& mv *.war temp/ \t&& cd temp \t&& mv cmdbuild*.war cmdbuild.war \t&& unzip cmdbuild.war \t&& mv cmdbuild.war /usr/local/tomcat/webapps/ROOT.war \t&& mv * /usr/local//tomcat/webapps/ROOT/ \t&& cd /opt/cmdbuild/extras/tomcat-libs/6.0 \t&& mv *.jar /usr/local/tomcat/lib \t&& cd /opt \t&& rm -rf *" shape="box"];
  "sha256:b3d97e740146a7de529c1e3ea4e92a3649eab87586c59aa35907514b9bb554ea" [label="sha256:b3d97e740146a7de529c1e3ea4e92a3649eab87586c59aa35907514b9bb554ea" shape="plaintext"];
  ...
  "sha256:a9f799e0b0f2ecc2969cd52cbd88d0cadc4d18c8e04891ff12fcbbba863529e7" -> "sha256:71b32f17a6f19a52fdc43dd003177093b5f113511928b5391dcfe40d1d2b059f" [label=""];
  "sha256:71b32f17a6f19a52fdc43dd003177093b5f113511928b5391dcfe40d1d2b059f" -> "sha256:b3d97e740146a7de529c1e3ea4e92a3649eab87586c59aa35907514b9bb554ea" [label=""];
}
